...
const AllBookmarks = () => {
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Are you sure you want to delete ${selectedBookmarks.length} bookmarks?`)) {
      try {
        await Promise.all(selectedBookmarks.map(id => bookmarkService.delete(id)));
        setBookmarks(prev => prev.filter(b => !selectedBookmarks.includes(b.Id)));
        setSelectedBookmarks([]);
        setShowBulkActions(false);
        toast.success(`${selectedBookmarks.length} bookmarks deleted successfully!`);
        await refreshSidebarData();
      } catch (error) {
  ...
  const handleBookmarkSelect = (bookmark) => {
    setSelectedBookmarks(prev => {
      const isSelected = prev.includes(bookmark.Id);
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-slate-50 to-blue-50">
      ...
      <div className="p-6">
        ...
        <AnimatePresence>
          {showBulkActions && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="mb-6 bg-primary/10 border border-primary/20 rounded-lg p-4"
            >
        ...
        {filteredBookmarks.length === 0 ? (
          <Empty
            title={hasFilters ? "No bookmarks found" : "No bookmarks yet"}
            description={hasFilters 
              ? "Try adjusting your filters or search terms to find what you're looking for."
              : "Start building your development resource library by adding your first bookmark."
            }
            onAction={hasFilters ? undefined : () => setIsAddModalOpen(true)}
            icon={hasFilters ? "Search" : "Bookmark"}
          />
        ) : (
          <motion.div
            layout
